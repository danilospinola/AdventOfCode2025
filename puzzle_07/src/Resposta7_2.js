const input = require('./input.js');
const array = input.split("\n")
let splits = []
let res = 0
let matrixSplits = []

for(x=0; x<array.length; x++){
    if(array[x].split("").filter(split => split == "^").length != 0){
        splits.push(array[x].split(""))
    }
}

for(x = 0; x<splits.length; x++){
    matrixSplits[x] = splits[x]
    for(y=0; y<splits.length[x]-1; y++){
       matrixSplits[x].push(splits[y])
    }
    
}

for(x = 0; x<matrixSplits.length; x++){
    for(y = 0; y<matrixSplits[x].length-1; y++){
        if(matrixSplits[x][y] == "^"){
            if(x== 0){
                matrixSplits[x][y-1] = 0
                matrixSplits[x][y+1] = 0 
            }
            else if(matrixSplits[x-1][y] == 0){
                matrixSplits[x][y-1] = 0
                matrixSplits[x][y+1] = 0 
            }
        } else if(x != 0 && matrixSplits[x-1][y] == 0  ){
            matrixSplits[x][y] = 0 
        }
    }
}


for(x = 0; x<matrixSplits.length; x++){
    for(y = 0; y<matrixSplits[x].length-1; y++){
        if(matrixSplits[x][y] == "^"){
            if(x== 0){
                matrixSplits[x][y-1] = 1
                matrixSplits[x][y+1] = 1 
            }
            else if(typeof matrixSplits[x-1][y] == 'number'){
                matrixSplits[x][y-1] += matrixSplits[x-1][y]
                matrixSplits[x][y+1] += matrixSplits[x-1][y] 
            }
        } else if(x != 0 && typeof matrixSplits[x-1][y] == 'number'  ){
            matrixSplits[x][y] += matrixSplits[x-1][y]
        }
    }
}

arrayResNumber = matrixSplits[matrixSplits.length-1].filter(split => typeof split == 'number')


for (let i = 0; i < arrayResNumber.length; i++) {
  res += arrayResNumber[i]; 
}

console.log(res)
